...
while k < 11:
    ...
    a = 0.00338
    mu = 0.004
    meters_in_pixel = 5 * 10 ** (-10)  # 2,625 * 10 ** (-11)
    Helium_diam_relative_max = 8
    dt = 10 ** (-11)  # seconds
    initial_velocity = 100 + k*1100/11  # meters / second
    ...
    V = ((width * height * depth) * meters_in_pixel ** 3)
    ...
    while running:
        ...
        if cycles % flag == 0:
            ...
            pendulum = 0
            theory_pressure = k_bolzman * T * population / V
            vdv_pressure = 8.31 * nu * T / (V - b * nu) - a * nu**2 / V**2
            print(pressure, "Pa, theory: ", theory_pressure, 'Pa, VdV:', vdv_pressure)
            temperatures.append(T)
            p_th.append(theory_pressure)
            p_exp.append(pressure)
            p_vdv.append(vdv_pressure)
            times.append(time)
    ...
    line2 = ax.plot(times, p_th, label='pressure from theory')
    line3 = ax.plot(times, p_vdv, label='pressure from Van der Vals')

    error = abs(sum(p_exp)/len(p_exp) - sum(p_th)/len(p_th))/(sum(p_th)/len(p_th))
    error_vdv = abs(sum(p_exp)/len(p_exp) - sum(p_vdv)/len(p_vdv))/(sum(p_vdv)/len(p_vdv))
    T = round(sum(temperatures)/len(temperatures), 2)
    ax.set(xlabel='time (s)', ylabel='pressure (Pa)', title='Helium, T = {0} K, error_th = {1}% error_vdv = {2}%'.format(
        str(T), str(round(error * 100, 2)), str(round(error_vdv * 100, 2))))
